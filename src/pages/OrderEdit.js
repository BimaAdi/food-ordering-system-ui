...
const OrderEdit = () => {
    ...
    useEffect(() => {
        ...
        let initialData = async () => {
            try {
                let menu = await axios.get(`${API.url}/menu`, {
                    headers: API.defaultHeader()
                });
                
                let order = await axios.get(`${API.url}/order/${id}`, {
                    headers: API.defaultHeader()
                });
                
                let initialOrderMenu = menu.data.data.map((x) => {
                  let orderMenu = order.data.order_menus.filter(y => y.menu_id === x.id);
                  if (orderMenu.length > 0) {
                    return {...x, counter: orderMenu[0].qty}
                  } else {
                    return {...x, counter: 0};  
                  }
                });
                
                setTableNumber(order.data.order_number);
                setMenus(initialOrderMenu);
            } catch (error) {
                console.error(error);
            }
        }
    ...
    let editOrder = () => {
        ...
        if (isValid === true) {
            ...
            axios.put(`${API.url}/order/${id}/`, requestJson, {
                headers: API.defaultHeader()
            })
            .then((res) => {
    ...
    return (
        <div className="docs-example-row">
            ...
            <div className="d-flex flex-row-reverse mb-2">
                ...
                <CButton color="success" className="ml-2" onClick={(e) => editOrder()}>Edit Order</CButton>
                <CInput 
